...
test_that("HclustDepart works as expected", {
  ...
  expect_true(all(HclustDepart(scppp_obj, maxSplit = 3)[["clust_results"]]$Hclust[[1]]$cluster[1:10]  == "1"))
  expect_true(all(HclustDepart(scppp_obj, maxSplit = 3)[["clust_results"]]$Hclust[[1]]$cluster[11:30]  == "2"))
  # p-value for first split less than 0.05
  expect_true(HclustDepart(scppp_obj, maxSplit = 3)[["clust_results"]]$Hclust[[2]][1:1] < 0.05)
})
